...
int main(void)
{
  ...
  // Buffer untuk terima 15 bytes binary
  uint8_t rx_buffer[15];
  uint8_t rx_index = 0;
  
  while (1)
  {
    // Terima 1 byte binary
    uint8_t received_byte;
    if(HAL_UART_Receive(&huart1, &received_byte, 1, 10) == HAL_OK){
      
      // Cek apakah ini start of packet (header byte 1)
      ...
      // Collect remaining bytes
        ...
        // Jika sudah 15 bytes, proses packet
        if(rx_index == 15){
          // Parse device data from bytes 6-15 (indices 5-14)
          // Bytes 2-4 (indices 2-4) are Discrete A, B, C (ignored for now)
          
          int data_index = 5;  // Start from byte 6 (index 5)
          
          for(int dev = 1; dev <= 5; dev++){
            if(dev == DEVICE_ID){
              uint8_t byte_high = rx_buffer[data_index];
              uint8_t byte_low = rx_buffer[data_index + 1];

              digital_val = (uint16_t)((byte_high << 8) | byte_low);
              
              // Different encoding based on device ID:
              // Device 1-2: Pitch/Roll - angle * 10
              // Device 3-4: Remote Heading/Course/Bearing/Compass - angle * 10
              // Device 5: Relative Course - (angle + 179.9) * 10
              
              if(DEVICE_ID == 5){
                // Relative Course encoding: (angle + 179.9) * 10
                // Decode: (value / 10) - 179.9
                // Range: -179.9° to +179.9°
                // NOTE: Raspi sends signed int16, so we need to convert from unsigned
                
                int16_t signed_val = (int16_t)digital_val;  // Convert to signed
                syncro_val = ((float)signed_val / 10.0f) - 179.9f;
              }
              else if(DEVICE_ID >= 1 && DEVICE_ID <= 4){
                // Standard encoding: angle * 10
                // Device 1: Pitch, Device 2: Roll
                // Device 3: Bearing, Device 4: Compass
              ...
              dsc(digital_val);  // Pass digital value directly, not degree

              // Blink LED (broadcast received) - Quick blink to avoid UART blocking
              HAL_GPIO_WritePin(LED_BLINK_GPIO_Port, LED_BLINK_Pin, GPIO_PIN_RESET);
              HAL_Delay(20);  // Reduced from 100ms - still visible but faster
              HAL_GPIO_WritePin(LED_BLINK_GPIO_Port, LED_BLINK_Pin, GPIO_PIN_SET);

              // Different label for each device
              if(DEVICE_ID == 1){
                sprintf(txt, "Pitch: %u (%.2f deg)\r\n", digital_val, syncro_val);
              }
              else if(DEVICE_ID == 2){
                sprintf(txt, "Roll: %u (%.2f deg)\r\n", digital_val, syncro_val);
              }
              else if(DEVICE_ID == 3){
                sprintf(txt, "Bearing: %u (%.2f deg)\r\n", digital_val, syncro_val);
              }
              else if(DEVICE_ID == 4){
                sprintf(txt, "Compass: %u (%.2f deg)\r\n", digital_val, syncro_val);
              }
              else if(DEVICE_ID == 5){
                sprintf(txt, "Rel.Course: %u (%.2f deg)\r\n", digital_val, syncro_val);
              }
              else{
                sprintf(txt, "Synchro#%d: %u (%.2f deg)\r\n", DEVICE_ID, digital_val, syncro_val);
              }
              HAL_UART_Transmit(&huart1, (uint8_t*)txt, strlen(txt), 100);
